...
processedData <- select(processedData, -empty, -rownum)
...
activityNew <- mutate(activity, weekdayend = weeker(date));

allIntervals <- unique(select(activityNew, interval));
stepsPerInterval <- data.frame();
for(x in allIntervals$interval) {
	stepsPerInterval <- rbind(stepsPerInterval, summarize(filter(activityNew, interval == x, !is.na(steps), weekdayend == "weekday"), mean(steps)));
}
weekdaySteps <- cbind(allIntervals, stepsPerInterval, "weekday");
colnames(weekdaySteps) <- cbind("interval", "intervalMean", "weekdayend")
weekdaySteps$interval <- sprintf("%04d", allIntervals$interval);

stepsPerInterval <- data.frame();
for(x in allIntervals$interval) {
	stepsPerInterval <- rbind(stepsPerInterval, summarize(filter(activityNew, interval == x, !is.na(steps), weekdayend == "weekend"), mean(steps)));
}
weekendSteps <- cbind(allIntervals, stepsPerInterval, "weekend");
colnames(weekendSteps) <- cbind("interval", "intervalMean", "weekdayend")
weekendSteps$interval <- sprintf("%04d", allIntervals$interval);

mergedWeeks <- rbind(weekdaySteps, weekendSteps);

library(lattice)
xyplot(intervalMean ~ as.numeric(interval) | weekdayend, mergedWeeks, layout = c(1,2), type="l")
```